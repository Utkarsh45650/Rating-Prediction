"""
Evaluate the trained CNN/LSTM model and calculate accuracy, precision, recall, F1 score
"""
...
def load_model(model_path='./cnn_lstm_rating_model'):
...
def prepare_data(file_path, sample_size=20000):
...
def evaluate_model(model_path='./cnn_lstm_rating_model', data_file='Reviews.csv', test_size=20000):
    ...
    # Load model
    model, vocab, max_length = load_model(model_path)
    ...
    # Predict
    predicted_labels, true_labels, probabilities = predict_batch(model, test_loader, device)
...
if __name__ == '__main__':
    # Configuration
    MODEL_PATH = './cnn_lstm_rating_model'
    DATA_FILE = 'Reviews.csv'
    TEST_SIZE = 20000  # Number of samples to test on
    ...
    try:
        ...
        print(f"\n❌ Error: {e}")
        print("Make sure the CNN model has been trained first!")
        print("Run: python train_cnn_lstm.py")
    except Exception as e:
